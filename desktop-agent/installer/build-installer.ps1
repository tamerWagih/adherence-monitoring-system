...
Write-Info "  # with verbose log (recommended for troubleshooting):"
Write-Info "  msiexec /i `"$msiFile`" /l*v `"$outputDir\install-$stamp.log`""
Write-Info ""
Write-Info "To uninstall:"
Write-Info "  # Preferred: uninstall the installed product by ProductCode (works even if you don't have the exact MSI you installed with)"
Write-Info "  # Run from an elevated PowerShell:"
Write-Info "  `$p = Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*','HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*' | ? { `$_.DisplayName -eq 'Adherence Monitoring Agent' } | Select-Object -First 1"
Write-Info "  if (`$p) { msiexec /x `$p.PSChildName /quiet /norestart /l*v `"$outputDir\uninstall-$stamp.log`" }"
Write-Info ""
Write-Info "  # If you are double-clicking an MSI in Explorer:"
Write-Info "  # - Only the *exact same MSI/ProductCode* can offer 'Remove' (maintenance mode)."
Write-Info "  # - Newer builds have a different ProductCode, so they will UPGRADE instead of uninstalling."
